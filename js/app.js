 ...
 document.body.innerHTML = document.body.innerHTML.replace(/\s([a-z])\s/g, ' $1&nbsp;');
          ...
          xmlhttp.onreadystatechange = function() {
              if (this.readyState == 4 && this.status == 200) {
                  ...
                  $("[data-target='#myModal']").on('click', function(){
                    ...
                    console.log('val=',$("#modalForm input[name='id']").val());
                  ...
                  $('a.eventTitle').on('click',function(){
                    ...
                    if ($eventDescription.hasClass('hiddenDescription') == false) {
                      $(".descriptionProgramEvent").addClass('hiddenDescription');
                      $("div.row.wyda").removeClass('shownDescription');
                    }
                    else {
                      $(".descriptionProgramEvent").addClass('hiddenDescription');
                      $("div.row.wyda").removeClass('shownDescription');
                      ...
                      $eventDescription.prev().addClass('shownDescription');
                    }
                    // $(this).next().fadeOut();
                  });
              }
  ...
  $('#modalForm div').on('click', function(){
    ...
    if (email == "") {
      ...
      $footer.text('Ilość biletów może być z zakresu od 1 do 6...');
    }
    else {

      // $('#reservationForm').trigger('submit');
      window.location.replace("php/rezerwacje.php?id="+id+"&email="+email+"&count="+count);
      // var $rezerwacja = this;
      ...
      //                 {
      //                   $('div.modal-footer').text("Email z potwierdzeniem został wysłany!"+this.responseText);
      //                   $($rezerwacja).hide();
      //                 }
      //               };
      //
  ...
  if (this.hash !== "") {
    ...
    $('html, body').animate({
      scrollTop: $(hash).offset().top-120 //120 is a size of nav bar
    }, 900, function(){
